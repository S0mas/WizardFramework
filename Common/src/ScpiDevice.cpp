...
void ScpiDevice::scpiCmd(const QString& cmd) const {
	unsigned long ret;
	connector_->call([this](auto vi, unsigned char* dataPtr, int size, unsigned long* ret) { return scpiIF_->write(vi, dataPtr, size, ret); }, QString("SCPI cmd sended: %1").arg(cmd), reinterpret_cast<unsigned char*>(cmd.toStdString().data()), strlen(cmd.toStdString().data()) + 1, &ret);
}
